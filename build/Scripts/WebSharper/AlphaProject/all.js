...
export function UserRegistration(){
  const serverRespons=_c_1.Create_1("");
  setTimeout(() => {
...
export function CarStatus(){
  ...
  const serverRespons=_c_1.Create_1("");
  const userPermission=_c_1.Create_1("");
  setTimeout(() => {
    userEmail().Set(globalThis.sessionStorage.getItem("userEmail"));
    globalThis.document.getElementById("LoginEmail").textContent=userEmail().Get();
...
function userEmail(){
  return _c_2.userEmail;
}
function password(){
  return _c_2.password;
}
...
function DecodeJson_FailureCost(){
...
function EncodeJson_CarJoinedData(){
  return Encoder_CarJoinedData?Encoder_CarJoinedData:Encoder_CarJoinedData=(EncodeRecord(void 0, [["car_licence", Id(), 0], ["user_id", EncodeBigInteger(), 0], ["c_type", Id(), 0], ["m_year", EncodeBigInteger(), 0], ["manuf", Id(), 0], ["failure", Id(), 0], ["repair_costs", Id(), 0], ["repair_status", Id(), 0]]))();
}
function DecodeJson_CarJoinedData(){
  return Decoder_CarJoinedData?Decoder_CarJoinedData:Decoder_CarJoinedData=(DecodeRecord(void 0, [["car_licence", Id(), 0], ["user_id", DecodeBigInteger(), 0], ["c_type", Id(), 0], ["m_year", DecodeBigInteger(), 0], ["manuf", Id(), 0], ["failure", Id(), 0], ["repair_costs", Id(), 0], ["repair_status", Id(), 0]]))();
}
function DecodeJson_RepairStatus(){
...
function LogingInToDatabase(userPassword, userEmail_1){
  return Bind((new AjaxRemotingProvider()).Async("Server/LogingInToDatabase", [userPassword, userEmail_1]), (o) => Return(o));
}
function GetFailureNames(){
  return Bind((new AjaxRemotingProvider()).Async("Server/GetFailureNames", []), (o) => Return(((DecodeList(DecodeJson_FailureCost))())(o)));
}
function CurrentUserId(password_1, email){
  return Bind((new AjaxRemotingProvider()).Async("Server/CurrentUserId", [password_1, email]), (o) => Return(o));
}
function InsertCarData(newCar){
  return Bind((new AjaxRemotingProvider()).Async("Server/InsertCarData", [(EncodeJson_CarJoinedData())(newCar)]), (o) => Return(o));
}
function GetUserPermission(userEmail_1){
  return Bind((new AjaxRemotingProvider()).Async("Server/GetUserPermission", [userEmail_1]), (o) => Return(o));
}
function GetCarData(userEMail, userPermission){
...
function Create(key, init_3){
  return CreateWithStorage(key, InMemory(ofSeq(init_3)));
}
...
let _c_2=Lazy((_i) => class $StartupCode_Client {
  ...
  static userEmail;
  static {
    this.userEmail=_c_1.Create_1("");
    this.password=_c_1.Create_1("");
  }
});
...
function InMemory(init_3){
  return new ArrayStorage(init_3);
}
...
function DecodeRecord(t, fields){
  return() =>(x) => {
    ...
    return t===void 0?o:Create_2(t, o);
  };
}
function DecodeBigInteger(){
  return() =>(x) => BigInt(x);
}
